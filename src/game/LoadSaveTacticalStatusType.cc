...
void ExtractTacticalStatusTypeFromFile(HWFILE const f, bool stracLinuxFormat)
{
	...
	UINT8 temp;
	EXTR_U8(d, temp); s->lastBattleSector.x = temp;
	EXTR_U8(d, temp); s->lastBattleSector.y = temp;
...
void InjectTacticalStatusTypeIntoFile(HWFILE const f)
{
	...
	INJ_U8(d, static_cast<UINT8>(s->lastBattleSector.x))
	INJ_U8(d, static_cast<UINT8>(s->lastBattleSector.y))
