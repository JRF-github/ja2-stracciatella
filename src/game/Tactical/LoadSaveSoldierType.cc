...
void ExtractSoldierType(const BYTE* const data, SOLDIERTYPE* const s, bool stracLinuxFormat, UINT32 uiSavedGameVersion)
{
	...
	*s = SOLDIERTYPE{};
	...
	EXTR_SKIP(d, 5)
	EXTR_TC(d, s->UpdateCounter)
	EXTR_TC(d, s->DamageCounter)
	EXTR_SKIP_I32(d)
	EXTR_SKIP(d, 4)
	EXTR_TC(d, s->AICounter)
	EXTR_TC(d, s->FadeCounter)
	...
	EXTR_SKIP(d, 2)
	EXTR_TC(d, s->BlinkSelCounter)
	EXTR_TC(d, s->PortraitFlashCounter)
	...
	EXTR_TC(d, s->PanelAnimateCounter)
	UINT32 checksum;
	EXTR_U32(d, checksum)
...
void InjectSoldierType(BYTE* const data, const SOLDIERTYPE* const s)
{
	...
	INJ_SKIP(d, 5)
	INJ_TC(d, s->UpdateCounter)
	INJ_TC(d, s->DamageCounter)
	INJ_SKIP_I32(d)
	INJ_SKIP(d, 4)
	INJ_TC(d, s->AICounter)
	INJ_TC(d, s->FadeCounter)
	...
	INJ_SKIP(d, 2)
	INJ_TC(d, s->BlinkSelCounter)
	INJ_TC(d, s->PortraitFlashCounter)
	...
	INJ_TC(d, s->PanelAnimateCounter)
