...
{
	hFile->write(giHabitatedDistance);
	hFile->write(giPopulationModifier);
	hFile->write(giLairID);
	hFile->write(gfUseCreatureMusic);
	hFile->write(giDestroyedLairID);
}


void LoadCreatureDirectives(HWFILE const hFile, UINT32 const uiSavedGameVersion)
{
	giHabitatedDistance = hFile->read<INT32>();
	giPopulationModifier = hFile->read<INT32>();
	giLairID = hFile->read<INT32>();
	gfUseCreatureMusic = hFile->read<BOOLEAN>();
	giDestroyedLairID = uiSavedGameVersion >= 82 ? hFile->read<INT32>() : 0;
