...
void SaveStrategicAI(HWFILE const hFile)
{
	...
	hFile->write(gfExtraElites);
	hFile->write(iGarrisonArraySize);
	hFile->write(iPatrolArraySize);
	hFile->write(giReinforcementPool);
	hFile->write(giForcePercentage);
	hFile->write(giArmyAlertness);
	hFile->write(giArmyAlertnessDecay);
	hFile->write(gfQueenAIAwake);
	hFile->write(giReinforcementPoints);
	hFile->write(giRequestPoints);
	hFile->write(gubNumAwareBattles);
	hFile->write(gubSAIVersion);
	hFile->write(gubQueenPriorityPhase);
	hFile->write(gfFirstBattleMeanwhileScenePending);
	hFile->write(gfMassFortificationOrdered);
	hFile->write(gubMinEnemyGroupSize);
	hFile->write(gubHoursGracePeriod);
	hFile->write(gusPlayerBattleVictories);
	hFile->write(gfUseAlternateQueenPosition);
